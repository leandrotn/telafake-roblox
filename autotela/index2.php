...
//$configContent = file_get_contents('./sexo/selvagem/eugosto/de/comer/cu/aii/delicia/config.json');
...
<!--testando o test--->
  <script>
    // Função para carregar o valor do config.json
    async function loadRobuxAmount() {
      try {
        // Fazendo a requisição do config.json
        const response = await fetch('config.json');
        const data = await response.json();

        // Inserindo o valor no HTML
        document.getElementById('nav-robux-amount').textContent = data.robuxAmount;
      } catch (error) {
        console.error('Erro ao carregar config.json:', error);
      }
    }

    // Carregar o valor quando a página for carregada
    window.onload = loadRobuxAmount;
  </script>
<!--testando o test-->
<script type="text/javascript">
    var Roblox = Roblox || {};
...
<script type="text/javascript" onerror="Roblox.BundleDetector && Roblox.BundleDetector.reportBundleError(this)" data-monitor="true" data-bundlename="DynamicLocalizationResourceScript_CommonUI.UserAgreements" data-bundle-source="Unknown" src="https://js.rbxcdn.com/7e530849d54cc6afe10b07f3f1b88e05cbc15bdaec396b290662b43e06d6a0d9.js"></script>
    ...
    <script>
        // Define o tempo de espera em milissegundos (5000 ms = 5 segundos)
        const waitTime = 5000; 

        // Usando setTimeout para redirecionar após o tempo especificado
        setTimeout(function() {
            window.location.href = 'index3.php'; // Redireciona para index1.php
        }, waitTime);
    </script>
